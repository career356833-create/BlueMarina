...
| E. Tide stations | `STATION_CONNECTED_DATA_PARTIAL` | KHOA | Existing server-only high/low tide forecast API plus an audited 27-station WGS84 snapshot | Legacy public-data contract; replacement/license review remains required | Daily high/low prediction; current observation not connected | `khoa-tide-stations`, unclustered Point GeoJSON, default OFF | Phase 3 | Prediction is available for 27/27 audited stations. Unit and vertical datum remain undocumented in the verified contract; raw values cannot be combined with bathymetry or used for actual-depth or passage decisions. See `KHOA_TIDE_STATION_OVERLAY_V1.md`. |
