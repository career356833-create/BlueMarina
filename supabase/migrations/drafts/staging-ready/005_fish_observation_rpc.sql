-- DRAFT ONLY. Atomic observation confirmation.
-- Transaction: migration yes; each function invocation is one PostgreSQL transaction.
-- Dependencies: 001-004.
begin;
create function public.guard_fish_observation_protected_fields() returns trigger
language plpgsql set search_path=pg_catalog,public,pg_temp as $$
begin
  if (new.species_id is distinct from old.species_id or new.verification_status is distinct from old.verification_status)
     and current_setting('fish.observation_mutation_context',true) is distinct from 'confirm_fish_observation' then
...
create trigger fish_observations_protected_fields before update of species_id,verification_status,visibility,deletion_status
on public.fish_observations for each row execute function public.guard_fish_observation_protected_fields();

create function public.confirm_fish_observation(
...
declare
  ...
  v_claimed_role text:=coalesce(auth.jwt()->'app_metadata'->>'fish_role','');
...
begin
  ...
  if v_claimed_role<>'' and v_role is null then
    raise exception using errcode='42501',message='fish_role_projection_required';
  elsif v_role='fish_crawler' then
    raise exception using errcode='42501',message='crawler_confirmation_forbidden';
  elsif v_role='fish_reviewer' and p_verification_type<>'expert_confirmed' then
    raise exception using errcode='42501',message='reviewer_requires_expert_confirmation';
  elsif v_role is null and p_verification_type<>'user_confirmed' then
    raise exception using errcode='42501',message='owner_requires_user_confirmation';
  elsif v_role is not null and v_role not in ('fish_admin','fish_reviewer') then
    raise exception using errcode='42501',message='unsupported_fish_role';
  end if;
  ...
  if v_role is null and v_observation.user_id<>v_caller then raise exception using errcode='42501',message='observation_owner_required'; end if;
  ...
  v_has_current:=found;
  if v_has_current and v_role is distinct from 'fish_admin' then
  ...
  if v_role is distinct from 'fish_admin' and (v_species.publish_status<>'published' or v_species.fact_review_status<>'approved') then raise exception using errcode='22023',message='species_not_published_for_confirmation'; end if;
  if v_role is distinct from 'fish_admin' and not exists(
  ...
  -- Serialize collection rebuilds for the affected user/species pairs. Sorting
  -- the keys keeps two concurrent admin corrections from deadlocking.
...
revoke all on function public.guard_fish_observation_protected_fields() from public,anon,authenticated;
