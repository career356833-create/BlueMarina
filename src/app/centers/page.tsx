...
function CenterCard({ center }: { center: MarineCenter }) {
  return (
    <article className="rounded-[1.5rem] border border-sky-100 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-xl font-black text-slate-950">{center.name}</h2>
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">{center.note}</p>
        </div>
        <span className="w-fit rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">{getStatusLabel(center.status)}</span>
      </div>
      ...
      <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
        ...
        <div className="rounded-2xl bg-slate-50 p-4 sm:col-span-2">
          ...
          <dd className="mt-2 flex flex-wrap gap-2">
            {(center.availableLicenses ?? []).map((licenseType) => (
...
export default function CentersPage() {
  ...
  const filteredCenters = useMemo(() => {
    ...
    return marineCenters.filter((center) => {
      ...
      const searchable = [center.name, center.region, center.city, center.address, center.phone, center.note].filter(Boolean).join(" ").toLowerCase();
  ...
  return (
    <PortalShell
      ...
      title="시험장 / 교육장 안내"
      description="필기시험장, 실기시험장, 수상안전교육장, 면제교육장을 지역과 유형별로 찾기 위한 검색 페이지 구조입니다."
    >
      <section className="rounded-2xl border border-sky-100 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-lg font-black text-slate-950">시험장/교육장 정보는 공식 자료 확인 후 순차 반영됩니다.</h2>
            <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">방문 전 공식 홈페이지 또는 해당 기관을 통해 반드시 확인하세요.</p>
          </div>
          <span className="w-fit rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">샘플 데이터 표시중</span>
        </div>
      </section>
      ...
      <section className="rounded-[2rem] border border-sky-100 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <label className="grid gap-2 text-sm font-black text-slate-700">
            지역 선택
            <select value={region} onChange={(event) => setRegion(event.target.value)} className="min-h-12 rounded-2xl border border-sky-100 bg-slate-50 px-4 text-sm font-bold text-slate-700 outline-none focus:border-sky-400">
              {regions.map((item) => (
          ...
          <label className="grid gap-2 text-sm font-black text-slate-700">
            시설 종류
            <select value={centerType} onChange={(event) => setCenterType(event.target.value as MarineCenterType | "all")} className="min-h-12 rounded-2xl border border-sky-100 bg-slate-50 px-4 text-sm font-bold text-slate-700 outline-none focus:border-sky-400">
          ...
          <label className="grid gap-2 text-sm font-black text-slate-700">
            면허 종류
            <select value={licenseType} onChange={(event) => setLicenseType(event.target.value as "all" | "general" | "yacht")} className="min-h-12 rounded-2xl border border-sky-100 bg-slate-50 px-4 text-sm font-bold text-slate-700 outline-none focus:border-sky-400">
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_0.72fr]">
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="rounded-full bg-sky-100 px-3 py-1 text-xs font-black text-sky-700">{filteredCenters.length}개</span>
          </div>
          ...
          {filteredCenters.length > 0 ? (
            ...
            <div className="rounded-[1.5rem] border border-sky-100 bg-white p-6 text-center shadow-sm">
              <p className="text-base font-black text-slate-950">검색 결과가 없습니다.</p>
        ...
        <aside className="h-fit rounded-[2rem] border border-sky-100 bg-[#0F2D52] p-5 text-white shadow-sm sm:p-6">
          ...
          <p className="mt-2 text-sm font-semibold leading-7 text-sky-100">좌표 데이터 확보 후 카카오맵 또는 네이버지도 연동 예정입니다.</p>
          <div className="mt-5 rounded-2xl bg-white/10 p-4">
            <div className="flex items-start gap-2">
              <ShieldCheck size={18} className="mt-0.5 shrink-0 text-sky-100" />
              <p className="text-sm font-semibold leading-6 text-sky-50">현재 샘플 데이터에는 실제 좌표와 외부 URL을 넣지 않았습니다.</p>
            </div>
          </div>
        </aside>
      </section>
