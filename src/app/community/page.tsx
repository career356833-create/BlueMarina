...
export default async function CommunityPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <AppFrame><section className="mx-auto max-w-6xl py-5 sm:py-10">
    ...
    {posts.length===0?<div className="mt-6 rounded-[26px] border border-dashed border-[#29465D] bg-[#071827] px-6 py-16 text-center"><MessageCircle className="mx-auto text-[#79C9D6]" size={34}/><h2 className="mt-4 text-xl font-black">등록된 커뮤니티 글이 없습니다</h2><p className="mt-2 text-sm font-semibold text-[#9FB3C8]">실제 사용자가 제출하고 검토된 글만 이 피드에 표시됩니다.</p></div>:<div className="mt-6 grid gap-4 sm:grid-cols-2">{posts.map(post=><article key={post.id} className="min-w-0 overflow-hidden rounded-[24px] border border-[#1F3A50] bg-[#071827]">{post.images[0]?<Image src={post.images[0].url} alt={post.images[0].alt??post.title} width={900} height={560} sizes="(max-width: 640px) 100vw, 50vw" unoptimized className="aspect-[16/10] w-full object-cover"/>:null}<div className="p-5"><p className="text-xs font-black text-[#79C9D6]">{communityPostTypeLabels[post.type]}{post.region?` · ${post.region.province} ${post.region.district??""}`:""}</p><h2 className="mt-2 break-words text-xl font-black">{post.title}</h2><p className="mt-3 break-words text-sm leading-6 text-[#B8CBDD]">{communityExcerpt(post.body)}</p><div className="mt-4 flex items-center justify-between gap-3"><time className="text-xs text-[#6E8299]" dateTime={post.createdAt}>{post.createdAt.slice(0,10)}</time><Link href={`/community/${encodeURIComponent(post.id)}`} className="inline-flex min-h-11 items-center px-2 text-sm font-black text-[#79C9D6]">상세보기</Link></div></div></article>)}</div>}
