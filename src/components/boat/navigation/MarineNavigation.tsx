    ...
    <main className="grid h-[100svh] min-h-[640px] w-full grid-rows-[64px_minmax(0,1fr)] overflow-hidden bg-[#06131a] text-[#f2eee3]">
      ...
      <div className="grid min-h-0 grid-rows-[minmax(280px,56%)_minmax(0,44%)] lg:grid-cols-[360px_minmax(0,1fr)] lg:grid-rows-1">
        ...
        <section className="relative order-1 min-h-0 overflow-hidden lg:order-2" aria-label="해상 내비게이션">
          ...
          <button type="button" onClick={toggleHud} aria-pressed={hudVisible} aria-label={`방향 HUD ${hudVisible ? "끄기" : "켜기"}`} className={`absolute right-3 top-3 z-[520] min-h-11 rounded-full border px-3 text-[10px] font-semibold tracking-[0.12em] shadow-lg backdrop-blur-md ${hudVisible ? "border-[#70d6dd]/70 bg-[#062229]/75 text-[#a9f4f1]" : "border-white/25 bg-[#06131a]/65 text-[#c0cac6]"}`}>HUD {hudVisible ? "ON" : "OFF"}</button>
          <button type="button" onClick={() => setMobileLayersOpen((current) => !current)} aria-expanded={mobileLayersOpen} aria-label={mobileLayersOpen ? "해양 레이어 닫기" : "해양 레이어 열기"} className="absolute right-3 top-16 z-[520] grid size-11 place-items-center rounded-full border border-white/25 bg-[#06131a]/65 text-[#dbe6e1] shadow-lg backdrop-blur-md sm:hidden">{mobileLayersOpen ? <X size={17} aria-hidden="true" /> : <Layers3 size={17} aria-hidden="true" />}</button>
