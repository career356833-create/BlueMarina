    ...
    <main className="grid h-[100svh] min-h-[640px] w-full grid-rows-[64px_minmax(0,1fr)] overflow-hidden bg-[#06131a] text-[#f2eee3]">
      ...
      <div className="grid min-h-0 grid-rows-[minmax(280px,56%)_minmax(0,44%)] lg:grid-cols-[360px_minmax(0,1fr)] lg:grid-rows-1">
        ...
        <section className="relative order-1 min-h-0 overflow-hidden lg:order-2" aria-label="해상 내비게이션">
          <NavigationMapShell presentation={{ vessel: effectiveVessel, destination, waypoints, track: visibleTrack?.points ?? [] }} deepWaterRouteVisible={deepWaterRouteVisible} harborZoneVisible={harborZoneVisible} navigationAidsVisible={navigationAidsVisible} trainingFiringZoneVisible={trainingFiringZoneVisible} navigationWarningsVisible={navigationWarningsVisible} tideStationsVisible={tideStationsVisible} marineWeatherVisible={marineWeatherVisible} navigationWarningFocus={navigationWarningFocus} onPointSelect={selectMapPoint} onDeepWaterRouteSelect={(properties: KhoaDeepWaterRouteProperties) => setSelectedMarineFeature({ kind: "deep-water-route", properties })} onHarborZoneSelect={(properties: KhoaHarborZoneProperties) => setSelectedMarineFeature({ kind: "harbor-zone", properties })} onNavigationAidSelect={(properties: KhoaNavigationAid) => setSelectedMarineFeature({ kind: "navigation-aid", properties })} onTrainingFiringZoneSelect={(properties: KhoaTrainingFiringZoneProperties) => setSelectedMarineFeature({ kind: "training-firing-zone", properties })} onNavigationWarningSelect={selectNavigationWarning} onTideStationSelect={(properties: KhoaTideStation) => setSelectedMarineFeature({ kind: "tide-station", properties })} onMarineWeatherSelect={(properties: KmaMarineWeatherForecastZone) => setSelectedMarineFeature({ kind: "marine-weather", properties })} onDeepWaterRouteStateChange={setDeepWaterRouteState} onHarborZoneStateChange={setHarborZoneState} onNavigationAidsStateChange={setNavigationAidsState} onTrainingFiringZoneStateChange={setTrainingFiringZoneState} onNavigationWarningsStateChange={setNavigationWarningsState} onTideStationsStateChange={setTideStationsState} onMarineWeatherStateChange={setMarineWeatherState} onNavigationWarningsDataChange={setNavigationWarningsData} />
