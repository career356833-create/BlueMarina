...
export type FishEcologySectionProps = {
  ecology?: string | null;
  ecologySourceStatus?: "present" | "source_missing";
  sourceLabel?: string;
  className?: string;
};
...
export function FishEcologySection({
  ecology,
  ecologySourceStatus,
  sourceLabel = DEFAULT_SOURCE_LABEL,
  className
}: FishEcologySectionProps) {
  if (!hasVisibleText(ecology) || ecologySourceStatus === "source_missing") {
    return null;
  }

  const paragraphs = splitParagraphs(ecology ?? "");
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <h3 className="text-lg font-bold text-slate-900 sm:text-xl">생태 정보</h3>
        </div>
...
export default FishEcologySection;
