...
export type FishFeedingSectionProps = {
  feeding?: string | null;
  feedingSourceStatus?: "present" | "source_missing";
  sourceLabel?: string;
  className?: string;
};
...
export function FishFeedingSection({
  feeding,
  feedingSourceStatus,
  sourceLabel = DEFAULT_SOURCE_LABEL,
  className
}: FishFeedingSectionProps) {
  if (!hasVisibleText(feeding) || feedingSourceStatus === "source_missing") {
    return null;
  }

  const paragraphs = splitParagraphs(feeding ?? "");
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <h3 className="text-lg font-bold text-slate-900 sm:text-xl">먹이 정보</h3>
        </div>
...
export default FishFeedingSection;
