...
test('legacy hairtail keeps its ID and source references outside the reviewed 38 registry', () => {
  const profile = load('src/lib/fishing-condition/factual-profile.ts').getFactualConditionProfile('BM-SPECIES-000444');
  assert.equal(profile.speciesId, 'BM-SPECIES-000444');
  assert.ok(profile.evidenceRefs.length > 0);
  assert.ok(profile.limitations.includes('LEGACY_PROFILE_OUTSIDE_REVIEWED_38'));
  const result = service.runStaticProfileReadModel(profile.speciesId, 9);
  assert.equal(result.profileContext.readiness, 'PROFILE_LIMITED');
  assert.equal(result.species.speciesId, profile.speciesId);
  assert.equal(result.availability, 'PARTIAL');
});
test('static request returns 200 partial without invoking a live provider', async () => {
  ...
  assert.ok(response.body.readModel.sourceStatus.every(s => s.reason === 'SOURCE_DISABLED' && s.status === 'DISABLED'));
});
...
test('configured sources are not claimed observed without a request; no decision keys', () => {
  ...
  assert.equal(configured.sourceStatus[0].status, 'UNKNOWN');
  assert.equal(configured.sourceStatus[0].reason, 'OBSERVATIONS_NOT_REQUESTED');
